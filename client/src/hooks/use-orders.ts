...
export function useOrderByCode(code: string) {
  return useQuery({
    queryKey: [api.orders.getByCode.path, code],
    queryFn: async () => {
      const url = buildUrl(api.orders.getByCode.path, { code });
      const res = await fetch(url, { credentials: "include" });
      if (res.status === 404) return null;
      if (!res.ok) throw new Error("Failed to fetch order");
      return api.orders.getByCode.responses[200].parse(await res.json());
    },
